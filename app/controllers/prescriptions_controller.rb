class PrescriptionsController < ApplicationController
  ...
  def create
    prescription = Prescription.new(drug: params[:prescription][:drug],
                    dosage_in_milligrams: params[:prescription][:dosage_in_milligrams],
                    appointment_id: params[:prescription][:appointment]
                  )
    if prescription.save
      redirect_to specialty_doctor_path(prescription.appointment.doctor.specialty, prescription.appointment.doctor)
    else
